...
public:
    ...
    /**
     *  Upon instantiation, a Task object receives a reference to an unmodifiable std::string.
     *  This string contains a single record from the task data file.
     *
     *  The constructor initializes the number of slots to a default value of 1 and the pointers 
     *  to the next tasks to safe addresses.  The object uses a Utilities constructor to extract 
     *  each token from the record (see below).  Once this constructor has extracted all of the 
     *  tokens from the record, it retrieves the maximum field width needed to align all extracted 
     *  tokens.  If that field width is greater than the field_width for all Task objects, this
     *  constructor resets the class field width to the value retrieved. (The display member 
     *  function uses this field width to align the output across all of the records retrieved 
     *  from the data file.)
     */
    ...
    /**
     *  attempts to validate a next task in the current object by comparing 
     *  the Task referenced in its parameter to the next tasks stored upon 
     ...
     *  the addressed of that next task has not been determined (by validate())
     */
